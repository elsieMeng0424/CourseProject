...
void FastPrinter::initDrawer() {
	...
	SetWindowPos(hwnd, HWND_TOP, 0, 0, int(cfi.dwFontSize.X * sizeX), int(cfi.dwFontSize.Y * sizeY), 0);
...
void FastPrinter::_draw() {
	...
	for (DWORD i = 0; i < sizeY; i++) {
		...
		WriteConsoleOutputCharacterA(hOutput, dataGrid + (i * sizeX), DWORD(sizeX), coordBufCoord, &dWrite);
	}
	SetConsoleActiveScreenBuffer(hOutput);
}
...
void FastPrinter::_drawC() {
	...
	DWORD dWrite = 0;
	WriteConsoleA(hOutput, outputSequence, DWORD(sequenceLength), &dWrite, NULL);
}
/******************************************************************
*  TO-DO END                                                      *
******************************************************************/
